...
recovered_people = []
dead_people = []
population = [healthy_people, infected_people, recovered_people, dead_people]
days = 0
...
def printgrid():
    ...
    for d in dead_people:
        dead_x.append(d.location_x)
        dead_y.append(d.location_y)
    ...
    plt.pause(0.0001)
    plt.scatter(dead_x, dead_y, color = 'grey', s = 3)        
    plt.pause(0.0001)
...
def draw():
    ...
    axes_1.plot(total_days, recovered,  label=f'Recovered: {len(recovered_people)}' ,color = 'blue')
    axes_1.plot(total_days, healthy, label=f'Healthy: {len(healthy_people)}',color = 'green')
    axes_1.plot(total_days, dead,color = 'grey', label=f'Dead: {len(dead_people)}')
    leg = axes_1.legend()
    plt.show()
...
def healing(days_until_healed, death_prob):
...
def infecting(radius, time_to_spread, initial_prob,days_until_healed, death_prob):
                ...
                for i in healthy_people:
                    if i.location_x < infection_line:
                        ...
                        if i.location_x > x.location_x + radius:
                            break
        healing(days_until_healed, death_prob)
        

        
def run_simulation(total_people, radius, days_until_healed, time_to_spread, initial_prob, infected_proportion, death_prob):
    create_room(infected_proportion, total_people)
    infecting(radius, time_to_spread, initial_prob, days_until_healed, death_prob)
    print(days)
    print(time.time() - tempo)
    draw()


run_simulation(total_people= 2000, radius=8, days_until_healed= 6, time_to_spread= 3, initial_prob=0.2, infected_proportion= 1/10, death_prob= 0.01)